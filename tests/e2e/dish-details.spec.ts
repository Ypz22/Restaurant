...
test('edita secciones desde admin y envía cocción y extras con el precio correcto', async ({ page }, testInfo) => {
  ...
  const info = dialog.locator('details').filter({ hasText: 'Información adicional' })
  ...
  expect(saved!.detail_sections[3].title).toBe('Origen del corte')
  ...
  const notesBounds = await page.getByRole('region', { name: 'Notas para la cocina', exact: true }).boundingBox()
  expect(infoBounds!.y).toBeGreaterThan(extraBounds!.y + extraBounds!.height)
  expect(notesBounds!.y).toBeGreaterThan(infoBounds!.y + infoBounds!.height)
